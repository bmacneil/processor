...
def trimPorts(pattern, ports, pDir, para):
    port = Ports()
    for p in ports:
        new_p = findParameters(p, para)
        size = getSize(new_p)
...
test_def_template = '''bool OUTPUT_PORT::{name}_test(std::string & msg) {{
    bool pass = true;
  if (!bitTest({name}, m_core->{name}, msg)) {{
    oss << "\\n    {name}_test FAILED: Expected: " << {name}.to_string() << " Output: " << int(m_core->{name});
    msg = oss.str();
    pass = false;
  }}
  return pass;
...
with open('include/port.hpp', 'w') as file:
    file.write(header)
    file.write('struct OUTPUT_PORT {VALUmux *m_core;\n')
    for port in outPorts:
        try:
...
print(maxPort)
block.nodesep = (3, 3)
block = block(r"ALUmux", alignment='nw')
height = ceil(maxPort)
